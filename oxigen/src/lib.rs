...
pub struct GeneticExecution<T: PartialEq + Send + Sync, Ind: Genotype<T>> {
    ...
    pub progress_log: (u64, Option<File>),
    ...
    impl<T: PartialEq + Send + Sync, Ind: Genotype<T>> GeneticExecution<T, Ind> {
        ...
        pub fn run_loop(&mut self) -> (u64, f64, Vec<Ind>) {
        ...
        pub fn print_progress_header(&mut self) {
    ...
    #[cfg(feature = "global_cache")]
    pub fn compute_fitnesses(&mut self, refresh_on_nocache: bool) {
